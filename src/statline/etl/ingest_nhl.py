...
def backfill_nhl(seasons: list[str]):
    ...
    with SessionLocal() as s:
        ...
        for season in seasons:  # '20232024', '20242025'
            for game_pk in nhl.iter_season_game_ids(season):
                box = nhl.get_boxscore(game_pk)
                teams = box.get("teams", {})
                home = teams.get("home", {}); away = teams.get("away", {})
                home_team_info = home.get("team") or {}; away_team_info = away.get("team") or {}

                home_team = _get_or_create_team(s, league.id, str(home_team_info.get("id") or f"home-{game_pk}"), home_team_info.get("name") or "HOME")
                away_team = _get_or_create_team(s, league.id, str(away_team_info.get("id") or f"away-{game_pk}"), away_team_info.get("name") or "AWAY")

                date = datetime.strptime(season[:4] + "-10-01", "%Y-%m-%d").replace(tzinfo=timezone.utc)
                game = _get_or_create_game(s, league.id, str(game_pk), int(season[:4]), date, home_team.id, away_team.id, status="Final")
                ...
                for side_key in ("home","away"):
                    ...
                    for sk_id in skaters:
                        ...
                        player = _get_or_create_player(s, league.id, p_ext, first, last or None, position=None, team_id=home_team.id if side_key=="home" else away_team.id)
